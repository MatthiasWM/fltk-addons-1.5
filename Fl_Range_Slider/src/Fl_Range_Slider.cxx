...
int Fl_Range_Slider::scrollvalue(int p, int n, int t, int l)
{
  ...
  if (horizontal()) {
    if (w() < 3 * h()) {
      ...
      minSash = l * minSash / (w() - 2 * h() - minSash);	// convert to size in 'lines'
    }
  } else {
    if (h() < 3 * w()) {
      minSash = 0;
      ...
      minSash = l * minSash / (h() - 2 * w() - minSash);
    }
  }
...
void Fl_Range_Slider::drawSlider(int x, int y, int w, int h)
{
  ...
  if (type() == FL_VERT_NICE_SLIDER) {
	...
	draw_box(pushed_ == 4 ? fl_down(box1) : box1, xsl + 2, ysl + 2,
	    hsl - 4, hsl - 4, selection_color());
	draw_box(pushed_ == 5 ? fl_down(box1) : box1, xsl + wsl - hsl + 2,
	    ysl + 2, hsl - 4, hsl - 4, selection_color());
      }
    } else {
      if (wsl > 6 && hsl >= wsl * 2.5f) {
	draw_box(pushed_ == 4 ? fl_down(box1) : box1, xsl + 2, ysl + 2,
	    wsl - 4, wsl - 4, selection_color());
	draw_box(pushed_ == 5 ? fl_down(box1) : box1, xsl + 2,
	    ysl + hsl - wsl + 2, wsl - 4, wsl - 4, selection_color());
      }
    }
  }
...
int Fl_Range_Slider::handle(int event)
{
  ...
  if (event == FL_PUSH) {
    ...
    sashWdt = (int) (slider_size() * W + 0.5f);
    double val =
	(maximum() - minimum())? (value() - minimum()) / (maximum() -
	minimum()) : 0.5;
    if (val > 1.0)
      sashPos = wdt - sashWdt;
    else if (val < 0.0)
    ...
    if (sashWdt < 2.5f * scaleWdt)	// scalers are not visible, so don't use them!
      ...
      pushed_ = 4;
      evVal = value();
      evSize = slider_size_i_;
    } else if (mouseDown >= sashPos + sashWdt - scaleWdt + 2
	&& mouseDown < sashPos + sashWdt) {
      pushed_ = 5;
      evVal = value();
      evSize = slider_size_i_;
    } else
...
handleEv:
  if (pushed_ > 3) {
	...
	if (horizontal()) {
	  ...
	  int nLines =
	      (int) (maximum() - minimum() + slider_size_i_ + min_sash_size_);
	  int wdt = w() - 2 * h();
	...
	if (pushed_ == 4) {
	  ...
	  if (evDelta < -evSize)
	    evDelta = -evSize;
	  scrollvalue(evVal - evDelta,
	      evSize + evDelta,
	      (int) minimum(), (int) (maximum() - minimum() + slider_size_i_)
	      );
	} else if (pushed_ == 5) {
	  if (evDelta > evSize)
	    evDelta = evSize;
	  if (evVal + evSize - evDelta >
	      maximum() - minimum() + slider_size_i_)
	    evDelta =
		(int) (-(maximum() - minimum() + slider_size_i_ - evVal -
		    evSize));
	  scrollvalue(evVal, evSize - evDelta, (int) minimum(),
	      (int) (maximum() - minimum() + slider_size_i_)
	      );
	}
